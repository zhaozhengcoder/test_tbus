...
static void * get_channel_config_pointer(int channel_index)
{
    ...
    return ((void * )shm_head) + shm_head->channels_config_begin_addr[channel_index];
}
...
static void * get_send_queue_begin_pointer(int channel_index,string src_addr)
{
    ...
    else
    {
        ...
        return ((void * )shm_head) + begin_addr;
    }
}
...
static void * get_channel_recv_queue_head_pointer(int channel_index,string src_addr)
{
    ...
    else
    {
        ...
        void * recv_head_pointer = (void *)shm_head + offset;
...
static int my_strncpy_two_segment_copy(char * des,char * sour,int msg_len,int total_len, int remain_send_size,void * send_queue_begin_addr)
{
    ...
    //拷贝剩下的一部分
    memcpy(send_queue_begin_addr,source+remain_send_size,total_len-remain_send_size);
    return 0;
}
...
static int handle_tbus_recv(string src_addr,string dest_addr,char * msg,int & msg_len)
{
    ...
    //判断空间是否够读
    if(remain_recv_size < sizeof(int))
    //剩余的空间小于int
    {
        //获得长度
        int * p_int = new int;
        ...
        memcpy(dest+remain_recv_size, recv_queue_begin_addr, sizeof(int) - remain_recv_size);
        msg_len = * p_int;
        //拷贝内容
        src = recv_queue_begin_addr + sizeof(int) - remain_recv_size;
    ...
    //剩余的空间大于int
    {
        ...
        if(msg_len > remain_recv_size-sizeof(int))
        {
            //如果后面的空间小于msg的长度，那么就要分成两个部分读
            void * dest = (void * ) msg;
            void * src  = ((void * ) send_head_pointer)+ sizeof(int);
            int memcpy_len1 = remain_recv_size-sizeof(int);
            memcpy(dest, src, memcpy_len1);
            memcpy(dest+memcpy_len1, recv_queue_begin_addr, msg_len-memcpy_len1);
        }
        else
        {
...
static int my_tbus_recv_all(string local_addr, string & peer_addr, char * msg,int & msg_len)
{
    ...
    for(int i=0;i<channel_src_vector.size();i++)
    {
...
bool is_can_send(int msg_len, int channel_index,string src_addr)
{
    ...
    else if(tail>head)
    {
        ...
        if(free_size > msg_len+sizeof(int))
        {
            return true;
        }
        return false;
...
int mytbus_send(string src_addr,string dest_addr,char * msg,int msg_len)
{
    ...
    {
        //判断是否需要分层两个部分写
        if(remain_send_size > (msg_len + sizeof(int)))
        {
